...
# Шары
sphere_idx = plotter.add_mesh(pv.Sphere(radius=0.1), color="red")   # красный шар — камера на нём
sphere_len = plotter.add_mesh(pv.Sphere(radius=0.1), color="blue")  # синий шар — движется по длине

# Активируем интерактивный режим
plotter.show(interactive_update=True)

# -------------------------
# Настройка оффсета камеры относительно шара
# -------------------------
camera_offset = np.array([0.0, -1.0, 0.3])  # смещение камеры позади шара
...
while True:
    ...
    for i in range(steps):
        ...
        # --- камера следует за красным шаром ---
        cam_pos = pos_idx + camera_offset
        plotter.camera_position = [cam_pos.tolist(), pos_idx.tolist(), [0, 0, 1]]
