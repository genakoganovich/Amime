...
# -----------------------------------------------------
# Создаем окно с двумя подкадрами
# -----------------------------------------------------
...
# =====================================================
# SUBPLOT 1 — Камера прикреплена к стрелке
# =====================================================
plotter.subplot(0, 1)
plotter.add_text("Camera attached to arrow", font_size=14)
...
while True:
    for i in range(STEPS):
        ...
        # -----------------------------------------
        # ПРАВОЕ окно (камера прикреплена к стрелке)
        # -----------------------------------------
        plotter.subplot(0, 1)
        ...
        # === Камера прикреплена к стрелке ===
        # камера находится чуть ПОЗАДИ стрелки и немного выше
        cam_offset = -direction * 0.6 + np.array([0, 0, 0.25])
        camera_position = pos + cam_offset

        plotter.camera.position = camera_position.tolist()
        plotter.camera.focal_point = (pos + direction * 1.0).tolist()
        plotter.camera.up = (0, 0, 1)
