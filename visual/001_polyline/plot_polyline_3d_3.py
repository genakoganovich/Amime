...
while True:
    line_cum, = ax2.plot([], [], marker='o', color='blue')  # сброс линии накопленной длины

    for i in range(1, len(trajectory) + 1):
        ax1.cla()  # очищаем 3D график

        # Отрисовываем сегменты траектории с окраской по длине
        for j in range(i - 1):
            seg = trajectory[j:j + 2]
            color_val = segment_lengths[j] / segment_lengths.max()
            ax1.plot(seg[:, 0], seg[:, 1], seg[:, 2], color=plt.cm.viridis(color_val), linewidth=3)
    ...
    time.sleep(1.0)
    # очищаем линии для следующей итерации
    ax1.cla()
    ax2.cla()
    ax2.set_xlim(0, len(cum_lengths) - 1)
    ax2.set_ylim(0, cum_lengths[-1] + 0.1)
